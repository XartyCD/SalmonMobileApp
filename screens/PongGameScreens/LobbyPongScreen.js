...
export default LobbyPongScreen = ( { navigation }) => { 
  ...
  const joinPongGame = (game_id, playerName) => {
    ...
    socket.on('startGame', (gameData) => {
      alert('Игра началась!', gameData);
      // Переход на экран игры и передача данных об игре
      navigation.navigate('GamePongScreenScreen', { gameData });
    });
            ...
            ponggames.map((game, index) => (
              <View key={index} style={styles.gameSessionBlock}>
                ...
                <Pressable
                  ...
                  onPress={() => joinPongGame(game.game_id, user)}
                >
